...
## Change column types
```{r}
CT_species_interest_time <- 
  CT_species_interest_time %>%
  dplyr::mutate(across(.cols = c(Species, Season, weekdays_binary, project_phase, lockdown),
                       .fns =  as.factor))
```

Add the project phase and lockdown to the table of the environmental variables
```{r}
...
## Rearrange columns
In bot the CT data and the environmental variables
```{r}
...
```{r}
CT_species_interest_time <- CT_species_interest_time %>%
  left_join(y = stacked_raster_values_and_garden_CT_all_seasons_no_nas[, c(1, 5:21)], by = "Station")
```

## Build a table only for squirrels
```{r}
CT_squirrels <- CT_species_interest_time %>%
  filter(Species == "Squirrel")
...
#                           "data-proc",
#                           "all_seasons",
#                           "CT_squirrels_30min_20230124.RDS"))
# 
# # stacked_raster_values_and_garden_CT_all_seasons_no_nas
