...
```{r}
data <- read.csv(here::here("R", "Scripts_Aimara", "act_by_hour_example.csv"))
# pct_rest_hour = n_rest_hour/ n_total_hour
...
(plot_light3 <- ggplot(ind_tmp) +
    geom_line(aes(x = hour, y = mean_pct), color = "red", linewidth = 2) + # , fill = mean_pct
    #coord_polar(start = 25) +
    theme_light() +
    ylab("percentage of resting time\n") +
    scale_fill_gradientn(colours = rev(my_bilbao)) +
